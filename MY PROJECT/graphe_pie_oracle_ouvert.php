<?php
	...
	$req="SELECT Count(`Numéro`) AS CompteDeNuméro,  AVG(`duree`)AS 'Durrée Avant cloture', `Affectation` FROM `table 1`WHERE ((`État` Not Like 'closed') AND (`Brève description` Like '%oracl%' Or `Brève description` Like '%gmao%')) OR ((`État` Not Like 'closed') AND (`Brève description` Like '%export%' And `Brève description` Like '%pdf%')) OR ((`État` Not Like 'closed') AND (`Brève description` Like '% AP %' Or `Brève description` Like '% FA %' Or `Brève description` Like '% GTA %')) OR ((`État` Not Like 'closed') AND (`Brève description` Like '%GPAO%' Or `Brève description` Like '% INV %' Or `Brève description` Like '% PO %' Or `Brève description` Like '% AR %')) OR ((`État` Not Like 'closed') AND (`Brève description` Like '% EAM %'))GROUP BY `Affectation`ORDER BY Count(`Numéro`)";	
...
<html>
	<head>
		<head>
		<title>graphe_pie_oracle_ouvert</title>
   ...
   <script src="js/script.js"></script>
	...
	<?php echo utf8_encode("<h1 style='text-align:center;'>graphe d'affectation Tickets Oracle ouvert</h1>");?>
